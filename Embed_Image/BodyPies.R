...
# Generate some random data.  In this case, 6 different body regions, generate random numbers 
# arbitrarily with mean = 0, sd = 1 
# assign random names to each column
...
### ### Relative abundance of taxa across body sites - pies
library(png)
library(grid)
library(scatterpie)

# First load in your png or jpg graphic upon which you want to plot your scatterplots
img <- readPNG( "aac_human_body.png")
# Then create a rasterGrob.  This allows you to include the png within
# the ggplot space.  If you change the width or height, you can alter
# the aspect ratio.  Keeping width and height = 1 retains the image
# aspect ratio
g<-rasterGrob(img, width=unit(0.8,"npc"), height=unit(1,"npc"), interpolate = FALSE)

# We'll use this rasterGrob below in the ggplot call, but first we need to decide
# where we want our pie charts to go.
# We do this by adding X, Y and radius coordinates to determine the size and
# placement of the pie charts (Ear, Genital, Gut, LH, Mouth, Nose)

# Add imX, imY and radius to our working data.frame, dat:
#dat$Region<-factor(c("ear","genital","gut","LH","mouth","nose"))
...
label_df<-data.frame(label=c(top))
